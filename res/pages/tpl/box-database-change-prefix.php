...
<?php
    ...
    $acx_dbRights = WwaInfoServer::getDatabaseUserAccessRights();
			...
			if (function_exists('wp_nonce_field')) {
...

             {
...

                 {
                     ...
                     $wwa_Message .= '<span class="acx-icon-alert-critical">'. __('The User which is used to access your Wordpress Database, hasn\'t enough rights (is missing the <code>ALTER</code> right) to alter the Table structure.
...

                 {
                     ...
                     $tables = WwaUtil::getTablesToAlter();
...

                     {
                         ...
                         $result = WwaUtil::renameTables($tables, $old_prefix, $new_prefix);
...

                         {
                             ...
                             $acxInfoMessage .= WwaUtil::renameDbFields($old_prefix, $new_prefix);



                             if (0 < WwaUtil::updateWpConfigTablePrefix($wpConfigFile, $new_prefix))
