...

{
...

    {
        ...
        if($keepMaxEntries < 1){
            ...
            $query = "TRUNCATE ".WwaPlugin::getTableName(WWA_PLUGIN_LIVE_TRAFFIC_TABLE_NAME);
        ...
        $optData = get_option('WWA_PLUGIN_ENTRIES_LIVE_TRAFFIC');
        ...
        $numEntries = $wpdb->get_var("SELECT COUNT(entryId) FROM ".WwaPlugin::getTableName(WWA_PLUGIN_LIVE_TRAFFIC_TABLE_NAME));



        if($numEntries <> $keepMaxEntries){

            update_option('WWA_PLUGIN_ENTRIES_LIVE_TRAFFIC', $numEntries);
        ...
        $tableName = WwaPlugin::getTableName(WWA_PLUGIN_LIVE_TRAFFIC_TABLE_NAME);
...

    {
        ...
        $query = $wpdb->prepare("INSERT INTO ".WwaPlugin::getTableName(WWA_PLUGIN_LIVE_TRAFFIC_TABLE_NAME)." (entryTime, entryIp, entryReferrer, entryUA, entryRequestedUrl)
        ...
        if(empty($optData)){
            ...
            add_option('WWA_PLUGIN_ENTRIES_LIVE_TRAFFIC', $numEvents);
        ...
        update_option('WWA_PLUGIN_ENTRIES_LIVE_TRAFFIC', $numEvents + 1);
...

    {
        ...
        if(empty($maxEntries)){
            ...
            return $wpdb->get_results("SELECT entryId,entryTime,entryIp,entryReferrer,entryUA,entryRequestedUrl FROM ".WwaPlugin::getTableName(WWA_PLUGIN_LIVE_TRAFFIC_TABLE_NAME)." ORDER BY entryId DESC");

        }

        else { return $wpdb->get_results("SELECT entryId,entryTime,entryIp,entryReferrer,entryUA,entryRequestedUrl FROM ".WwaPlugin::getTableName(WWA_PLUGIN_LIVE_TRAFFIC_TABLE_NAME)." ORDER BY entryId DESC LIMIT 0, ".$maxEntries);}
...

    {
        ...
        return $wpdb->get_var("SELECT entryId FROM ".WwaPlugin::getTableName(WWA_PLUGIN_LIVE_TRAFFIC_TABLE_NAME)." ORDER BY entryId DESC");
...

    {
        ...
        if ( !wp_verify_nonce( $_REQUEST['nonce'], "wwaTrafficScan_nonce")) { exit(__('Invalid request - nonce')); }
        ...
        if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            ...
            if(empty($data)){ $result['data'] = '<tr><td><p style="margin: 5px 5px; font-weight: bold; color: #cc0000">No data yet.</p></td></tr>'; }
...

                {
                    ...
                    $result['data'] .= '<tr><td class="wwa-scan-entry">';
