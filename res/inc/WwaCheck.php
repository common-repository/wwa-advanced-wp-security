...
class WwaCheck extends WwaPlugin

{
                ...
                if(in_array($userRole, array(8,9,10))){
                    ...
                    self::alert($actionName, $alertType, WWA_PLUGIN_ALERT_CRITICAL,
                        ...
                        sprintf(__('<p>One well known and dangerous WordPress security vulnerability is User Enumeration, in which a malicious user is able to enumerate
                            ...
                            <p>In order to protect from this type of attack, you should change your default <a href="%s" target="%s">WordPress administrator</a>

                            username to something more difficult to guess.</p>'), 'http://www.worldwebarts.com/', '_blank'),
...

    {
        ...
        $actionName = $wwaPluginAlertsArray['check_table_prefix']['name'];

        $alertType = $wwaPluginAlertsArray['check_table_prefix']['type'];
        ...
        if(strcasecmp('wp_', $wpdb->prefix)==0){
            ...
            self::alert($actionName, $alertType, WWA_PLUGIN_ALERT_CRITICAL,
                ...
                sprintf(__('<p>This plugin can automatically <a href="%s">do this</a> for you, but if you want to do it manually then please read this <a href="%s" target="%s">article</a> first.</p>'),
                        ...
                        'admin.php?page=wwa_database',

                        'http://www.worldwebarts.com/', '_blank')
...

    {
        ...
        $actionName = $wwaPluginAlertsArray['check_wp_current_version']['name'];

        $alertType = $wwaPluginAlertsArray['check_wp_current_version']['type'];
...

    {
        ...
        $actionName = $wwaPluginAlertsArray['check_index_wp_content']['name'];

        $alertType = $wwaPluginAlertsArray['check_index_wp_content']['type'];
        ...
        $actionName = $wwaPluginAlertsArray['check_index_wp_plugins']['name'];

        $alertType = $wwaPluginAlertsArray['check_index_wp_plugins']['type'];
        ...
        $actionName = $wwaPluginAlertsArray['check_index_wp_themes']['name'];

        $alertType = $wwaPluginAlertsArray['check_index_wp_themes']['type'];
        ...
        $actionName = $wwaPluginAlertsArray['check_index_wp_uploads']['name'];

        $alertType = $wwaPluginAlertsArray['check_index_wp_uploads']['type'];
        ...
        $actionName = $wwaPluginAlertsArray['check_htaccess_wp_admin']['name'];

        $alertType = $wwaPluginAlertsArray['check_htaccess_wp_admin']['type'];
        ...
        $actionName = $wwaPluginAlertsArray['check_readme_wp_root']['name'];

        $alertType = $wwaPluginAlertsArray['check_readme_wp_root']['type'];
